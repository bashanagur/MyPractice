...
import com.merchant.StartMerchantsGuide.RomanNumbers.Roman;
...
public class ReadInputAndProcess {
  ...
  /**
   * Parses the input line by line and decides the type of request appropriately and forwards the request
   * @param line
   * @throws Exception
   */
   ...
   * maps the planet language to the RomanType
   * @param arr
   */
  ...
  private int generateGalacticUnitToNumericValue(String[] arr){
      ...
      try {
      String romanNumeral = generateRomanFromGalaxyUtil(arr);
  ...
  /**
   * a utils method to generate Roman string from Planet input 
   * @param arr
   * @return
   */
  private String generateRomanFromGalaxyUtil(String[] arr){
      StringBuilder romanNumeral = new StringBuilder();
      int i;
      for(i = 0; i< arr.length; i++){
          Roman roman = interGalacticRomanMapping.get(arr[i]);
